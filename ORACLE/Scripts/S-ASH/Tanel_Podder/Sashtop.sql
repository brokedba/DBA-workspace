...
--------------------------------------------------------------------------------
--
-- File name:   Sashtop.sql v1.2
-- Purpose:     Display top ASH time (count of ASH samples)from licence free Simulated ASH [http://pioro.github.io/orasash/]  grouped by your
...
--               4- Added dbid join between the tables in case many target DBs are configured 
--                 (Mak sure you select the right dbid first using switchdb.sql -- all target DBs info are in sash_targets)                 
-- Usage:       
--     @ashtop <grouping_cols> <filters> <fromtime> <totime>
--
-- Examples:
--     @ashtop username,sql_id session_type='FOREGROUND' sysdate-1/24 sysdate
--     @ashtop event2,session_id||','||session_serial#,sql_id,blocking_session "event='enq: TX - row lock contention'" DATE'2018-09-04' DATE'2018-09-11'
-- Other:
...
--    , MAX(sql_exec_id) - MIN(sql_exec_id) 
    ...
    AND u.dbid=a.dbid AND o.dbid=a.dbid
    GROUP BY
        &1
    ORDER BY
        TotalSeconds DESC
